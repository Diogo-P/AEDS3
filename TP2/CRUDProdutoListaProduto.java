...
public class CRUDProdutoListaProduto {
    ...
    public static void main(String[] args) {
        try {
            ...
            do {
                ...
                System.out.println("6. Atualizar associacao (quantidade/observacoes)");
                System.out.println("7. Remover associacao");
    ...
    private static void adicionarProdutoNaLista() throws Exception {
        ...
        System.out.println("Produto adicionado a lista! Associacao ID = " + id);
    }
    ...
    private static void listarListasDeUmProduto() throws Exception {
        ...
        if (relacoes == null || relacoes.length == 0) {
            System.out.println("Produto nao esta em nenhuma lista.");
            return;
        }
        for (ListaProduto lp : relacoes) {
            System.out.printf("Associacao ID=%d | Lista ID=%d | qtd=%d | obs=%s%n",
                    lp.getID(),
                    lp.getIdLista(),
                    lp.getQuantidade(),
                    lp.getObservacoes() != null ? lp.getObservacoes() : "(nenhuma)");
    ...
    private static void atualizarListaProduto() throws Exception {
        System.out.print("ID da associacao (ListaProduto): ");
        ...
        System.out.print("Nova observacao (" + (lp.getObservacoes() != null ? lp.getObservacoes() : "") + "): ");
        String obs = sc.nextLine();
        if (!obs.isBlank()) lp.setObservacoes(obs);
        if (arqListaProduto.update(lp)) System.out.println("Associacao atualizada!");
    ...
    private static void removerAssociacao() throws Exception {
        System.out.print("ID da associacao (ListaProduto): ");
        int id = Integer.parseInt(sc.nextLine());
        if (arqListaProduto.delete(id)) System.out.println("Associacao removida!");
