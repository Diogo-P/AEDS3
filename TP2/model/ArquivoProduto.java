...
public class ArquivoProduto extends Arquivo<Produto> {
    ...
    public Produto readGtin(String gtin13) throws Exception {
        // Tentar primeiro a pesquisa por indice (caminho rapido)
       ...
       // Alternativa (Fallback):indice nao encontrado (possivel se o indice foi construido com normalizacao diferente ou esta corrompido)
    // Realizar uma varredura linear comparando GTINs normalizados (somente digitos)
        ...
        Arrays.sort(
            ...
            Comparator.nullsLast( // se o objeto Lista for null, manda pro final
                Comparator.comparing(
                    Produto::getNome,
                    Comparator.nullsFirst(String.CASE_INSENSITIVE_ORDER) // se o nome for null, fica no inicio
                )
            )
        );
        ...
        if (super.update(novoProduto)) {
            if (!novoProduto.getGtin13().equals(gtinAntigo)) {
                try {
                    ...
                    if (GTIN_INDEX_VERBOSE) System.err.println("Aviso: falha ao excluir o indice GTIN antigo para'" + gtinAntigo + "' -> " + e.getMessage());
                }
                try {
                    indiceIndiretoGtin.create(new ParGtinID(novoProduto.getGtin13(), novoProduto.getID()));
                } catch (Exception e) {
                    if (GTIN_INDEX_VERBOSE) System.err.println("Aviso: falha na criacao do indice GTIN durante a atualizacao para GTIN='" + novoProduto.getGtin13() + "' ID=" + novoProduto.getID() + " -> " + e.getMessage());
                }
            }
    ...
    /* METODO QUE NAO DEVE SER USADO MAS NaO DELETAR O COMENTaRIO
    @Override
