...
public class HashExtensivel<T extends RegistroHashExtensivel> {
  ...
  public class Cesto {
    ...
    Constructor<T> construtor;
    short quantidadeMaxima; // quantidade maxima de elementos que o cesto pode conter
    ...
    ArrayList<T> elementos; // sequencia de elementos armazenados
    ...
    public boolean create(T elem) {
      if (full())
        return false;
      int i = quantidade - 1; // posicao do ultimo elemento no cesto
  ...
  protected class Diretorio {
    ...
    // so serao considerados valores absolutos da chave.
    protected int hash(int chave) {
      return Math.abs(chave) % (int) Math.pow(2, profundidadeGlobal);
    }

    // Metodo auxiliar para atualizar endereco ao duplicar o diretorio
  ...
  public boolean create(T elem) throws Exception {
    ...
    // Carrega TODO o diretorio para a memoria
